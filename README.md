...
The current architecture can be extended with additional tools.
...
This would allow BookWise to research information beyond the Books to Scrape catalogue.
...
This would allow research tasks such as:

```text
Find recent papers on Hindi-English code-switched ASR.
```
...
These capabilities require appropriate search and retrieval tools to be integrated into the agent.

---

# Important Design Principle

BookWise should **never claim to have researched information that it cannot actually retrieve**.

For example, if the user asks:

```text
Find recent papers about Hindi-English code-switched speech recognition.
```

and no academic or web search tool is available, the correct behavior is to explain that the required information source is unavailable.

This keeps the system grounded in actual evidence rather than generated assumptions.

> **Grounded research requires grounded tools.**
...
if generated output files are not intended to be committed.
